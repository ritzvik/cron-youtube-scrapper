...
def set(key, data, duration, *args, **kwargs):
    cache.set('%s_%s' % (settings.CACHE_PREFIX, key.format(*args, **kwargs)), data, duration)
...
def incr(key, delta=1, *args, **kwargs):
    cache.incr('%s_%s' % (settings.CACHE_PREFIX, key.format(*args, **kwargs)), delta=delta)


def decr(key, delta=1, *args, **kwargs):
    cache.decr('%s_%s' % (settings.CACHE_PREFIX, key.format(*args, **kwargs)), delta=delta)
